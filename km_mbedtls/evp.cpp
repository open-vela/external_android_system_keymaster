...
RSA* EVP_PKEY_get1_RSA(EVP_PKEY* pkey) {
    mbedtls_pk_context* mbedtls_pkey = reinterpret_cast<mbedtls_pk_context*>(pkey);
    if (pkey == nullptr || mbedtls_pk_get_type(mbedtls_pkey) != MBEDTLS_PK_RSA) {
        return nullptr;
    }

    mbedtls_rsa_context* rsa = mbedtls_pk_rsa(*mbedtls_pkey);
    return reinterpret_cast<RSA*>(rsa);
}
